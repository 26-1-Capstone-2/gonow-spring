...
| 약속 제목(후순위) | title | VARCHAR(100) | - | 예: 면접 스터디 |
| 목적지 주소 | dest_address | VARCHAR(255) | NOT NULL | 약속 장소 |
| 목적지 위도 | dest_lat | DECIMAL(10, 8) | NOT NULL | - |
| 목적지 경도 | dest_lng | DECIMAL(11, 8) | NOT NULL | - |
| 목표 시간 | target_time | DATETIME | NOT NULL | - |
| 약속 알람 스위치 | is_active | BOOLEAN | NOT NULL, DEFAULT TRUE | 약속 활성화 ON/OFF |
| 약속 상태 | status | ENUM | NOT NULL, DEFAULT 'ACTIVE' | READY(방을 만든 직후), ACTIVE(한명이라도 이동 중이면), FINISHED(모든 친구가 도착하면) |
...
| 출발지 위도(후순위) | origin_lat | DECIMAL(10, 8) | - | 출발 시점의 위도 |
| 출발지 경도(후순위) | origin_lng | DECIMAL(11, 8) | - | 출발 시점의 경도 |
...
| 참여자 상태 | status | ENUM | NOT NULL, DEFAULT 'READY' | READY(대기 중), MOVING(이동 중), ARRIVED(도착 완료) |
| 알람 수신 여부(후순위) | is_alarm_on | BOOLEAN | NOT NULL, DEFAULT TRUE | push 알람을 받을지 말지 (임시 컬럼이므로 NULL로 허용하자) |
...
| 여정 제목(후순위) | title | VARCHAR(100) | - | 예: 기차역, 알바 가는 길 |
...
| 목적지 위도 | dest_lat | DECIMAL(10, 8) | NOT NULL | - |
| 목적지 경도 | dest_lng | DECIMAL(11, 8) | NOT NULL | - |
| 막차 여부 | is_last_mode | BOOLEAN | NOT NULL, DEFAULT FALSE | TRUE면 막차 시간 기준, FALSE면 직접 입력 기준 |
| 목표 시간 | target_time | DATETIME | NOT NULL | 직접 입력값 혹은 API로 가져온 막차 시각 |
| 도착 예정 시간 | estimated_arrival | DATETIME | - | 서버가 계산한 도착 예정 시간 |
| 반복 요일 | repeat_days | INT | NOT NULL, DEFAULT 0 | 요일 반복 비트마스크 (0: 안 함) |
| 여정 알람 스위치 | is_active | BOOLEAN | NOT NULL, DEFAULT TRUE | 여정 활성화 ON/OFF |
| 여정 상태 | status | ENUM | NOT NULL, DEFAULT 'READY' | READY(대기 중), MOVING(이동 중), ARRIVED(도착 완료) |
