...
### 1. 현재 위도/경도(`current_lat`/`current_lng`, `Journey`/`Participant`의 `currentPoint`)
...
### 2. MOVING 상태 — 플라스크(외부 API) 무조건 호출 문제 (Redis와 독립적으로 먼저 할 수 있는 개선)
