...
## 0. 테스트 대상 앱 상태 — 정책 (2026-08-18 확정)

실기기 테스트는 항상 아래 **3가지 앱 상태**만 검증한다.

| 상태 | 의미 |
|---|---|
| **포그라운드** | 앱을 열어서 화면이 보이는 상태 |
| **백그라운드** | 홈 버튼 등으로 앱을 최소화 — 프로세스는 살아있음(FGS가 떠 있으면 특히 그렇다) |
| **스와이프(모두 닫기)** | 최근 앱 목록에서 위로 스와이프해서 앱을 제거 |

**설정 앱 → 강제 종료(Force Stop)는 테스트 대상에서 제외한다** — 실제 사용자가 앱을 직접 강제 종료하는 일은 거의 없기 때문에, 이 경로까지 굳이 검증할 실익이 없다고 판단.

**주의 — 스와이프해도 프로세스가 안 죽을 수 있다**: FGS(포그라운드 서비스, 상단바 "위치 추적 중" 알림)가 떠 있는 동안은 안드로이드가 그 프로세스를 의도적으로 안 죽인다(FGS의 존재 이유 자체가 "이 프로세스는 지금 중요한 일을 하는 중이니 OS가 함부로 안 죽이게" 하는 것). 그래서 스와이프 테스트를 해도 `adb logcat`에 찍히는 PID가 바뀌지 않고, 로그 흐름도 "완전히 새로운 헤드리스 JS 인스턴스"가 아니라 "프로세스는 살아있지만 앱 UI만 백그라운드"인 경로(`AlarmManager.forgetIfExists()`가 좀비 러너를 정리하는 바로 그 케이스)를 타는 게 정상이다 — 이걸 버그로 오해하지 말 것(2026-08-18, 버그45 검증 중 실기기로 확인).
