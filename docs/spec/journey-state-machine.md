...
GPS는 원칙적으로 지오펜싱 대신 **주기적 폴링(N초마다)** 방식을 사용한다. (Apple Developer 계정 없이 iOS 개발 시 지오펜싱 불가) 단, 안드로이드는 전 상태 지오펜싱 전환이 코드상 완료됐다 — NEARDEST(2026-08-12, 순수 지오펜싱) → DEPARTING(2026-08-17, 순수 지오펜싱) → MOVING(2026-08-17, 폴링과 병행하는 보조 지오펜싱) → READY(2026-08-17, 지오펜싱 + 서버 시간 트리거) 순서로 구현됨. READY는 실기기 검증 전(2026-08-17 기준, 나머지는 검증·커밋 완료). iOS는 모든 상태에서 기존 폴링 방식 그대로다. 상세 설계와 진행 상황은 `docs/planning/geofencing-migration-plan.md` 참고.
