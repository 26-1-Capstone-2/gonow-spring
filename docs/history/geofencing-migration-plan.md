...
>
> **마이그레이션 완료(2026-08-17)**: **READY까지 구현·실기기 검증 완료 — NEARDEST/DEPARTING/MOVING/READY 4개 상태 전부 완료됨.** 서버 `DepartingTransitionScheduler`(READY→DEPARTING 시간 트리거) 신설 + 프론트 `readyGeofenceTask.ts`(앵커 500m EXIT 재센터링 + 목적지 100m ENTER→NEARDEST 핸드오프) 신설. 착수 전 발견한 반복 여정 스테일 값 버그(버그44)를 먼저 고친 뒤 진행. 실기기 검증(개인/귀가/그룹, 포그라운드/백그라운드/스와이프 전 조합) 중 상태머신 로직과는 별개로 "도착 확인"·FGS 생명주기 레이어에서 경쟁 조건 5건 + 콜드부팅 시 알림 액션 누락 1건, 총 6건의 프론트 버그를 추가로 발견·수정했다 — 상세는 `docs/history/resolved-bugs.md`의 "지오펜싱 Phase 3(READY) 실기기 테스트 중 발견·수정한 프론트 버그 6건" 참고. 같은 계기로 알람 서브시스템 8개 파일의 `console.log`를 전부 `dlog`로 통일(181개 전환) — 이번 6건 중 여러 개가 "이 지점은 dlog가 없어서 원인 파악에 시간이 걸렸다"는 공통 패턴으로 발견됐기 때문. 백엔드/프론트 커밋은 이 문서 갱신 직후 별도로 진행.
...
| 상태 | 현재 방식 | 전환 후 |
|---|---|---|
| READY | 폴링(최대 300초) | **완료(2026-08-17) — 지오펜싱(앵커 500m EXIT 재센터링 + 목적지 100m ENTER) + 서버 시간 트리거(`DepartingTransitionScheduler`), 실기기 검증 완료(개인/귀가/그룹, 앵커 재센터링 2회 이상·NEARDEST↔READY 왕복·DEPARTING 핸드오프까지 전부 확인).** `readyGeofenceTask.ts`. 착수 전 발견해 먼저 고친 관련 버그: 반복 여정 앵커/출발시각 스테일 값(버그44). 검증 과정에서 도착확인/FGS 생명주기 레이어의 별도 버그 6건 추가 발견·수정(`docs/history/resolved-bugs.md` "지오펜싱 Phase 3(READY) 실기기 테스트 중 발견·수정한 프론트 버그 6건" 참고) |
...
3. **READY** — 앵커 재센터링 + `P >= Q` 시간 트리거용 신규 인프라가 필요해 가장 마지막. **완료(2026-08-17), 실기기 검증 완료** — 서버 `DepartingTransitionScheduler`(신규) + 프론트 `readyGeofenceTask.ts`(신규, 앵커 500m EXIT마다 재센터링 + 목적지 100m ENTER로 NEARDEST 핸드오프) + `backgroundAlarmTask.ts`(`sync_event: departing_transition` 수신 시 캐시된 앵커로 DEPARTING 지오펜스 핸드오프). 플랫폼 분기는 별도 디스패처 모듈 없이 각 진입점(`alarmService.ts`/`backgroundLocationTask.ts`)에 `Platform.OS === 'android'` 인라인 체크로 처리 — NEARDEST/DEPARTING/MOVING에도 소급 적용 완료, 4개 상태 전부 통일됨.
...
- ~~반복 알람이 ARRIVED를 지나도 FGS를 유지하도록 `AlarmRunner` 생명주기 재설계~~ → **버그45로 별도 추적, 구현·실기기 검증 완료(2026-08-18)** — 이 문서는 지오펜싱 마이그레이션 완료 시점(2026-08-17)의 스냅샷이라 이 항목을 당시 최우선 미구현 TODO로 남겨뒀으나, 이후 `BUGS.md`(버그45)에서 별도 작업으로 진행돼 완료됐다. 상세는 `BUGS.md` 버그45 참고.
- ~~SCHEDULED/READY 등 급하지 않은 대기 구간의 GPS 폴링 주기 최적화~~ → **안드로이드는 Phase 3로 자연 해소(2026-08-17)** — 이 항목은 READY가 아직 폴링 기반이던 시절(Phase 3 착수 전)에 적어둔 것인데, READY가 지오펜싱으로 완전히 대체되면서 대기 구간에 GPS 폴링 자체가 안 도는 구조가 됐다(폴링 주기를 늘리고 말고 할 대상이 없어짐). 서비스가 안드로이드 전용이라 그대로 해소로 간주. iOS는 여전히 폴링 기반이라 이 최적화가 유효하지만, iOS 빌드 자체를 안 하고 있어 당장 착수 대상 아님.
...
- ~~DEPARTING/READY 전환 후 `fallbackToPolling()`의 "최후 안전망" 재설계~~ → **재검토 후 현행 유지로 확정(2026-08-17)** — "5분 제한 재시도 후 포기하고 서버 스케줄러(1시간 초과 자동 ARRIVED)에 위임" 안을 검토했으나, 그러면 5분~1시간 사이 클라이언트가 추적을 완전히 놓치는 방치 구간이 생긴다는 단점이 채택 기준을 못 넘었다. 지금처럼 무기한 폴링 폴백을 유지하면 방치 구간 없이(첫 성공 응답이 오는 즉시 자동으로 지오펜스 복귀) 배터리 손해도 실측상 극히 드문 케이스(GPS+서버 동시 지속 실패)에서만 발생해 감수할 만하다고 판단, 코드 변경 없음.
