...
> ⚠️ **구버전 설계 문서입니다.** 초기 설계안(v1.0) 기준이며 일부 내용(예: 매 1분 실시간 알람 감시 스케줄러)은 실제 구현과 다를 수 있습니다. 실제 동작은 루트 `CLAUDE.md`의 "스케줄러", "알람 구현 방식 결정 사항" 섹션을 우선 확인하세요. 이 문서는 설계 배경 참고용으로만 보관합니다.
