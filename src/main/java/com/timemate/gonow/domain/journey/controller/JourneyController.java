...
public class JourneyController {
    ...
    public ApiResult<JourneyResponse> createPersonalJourney(
            @MemberId Long memberId,
            @Valid @RequestBody PersonalJourneyCreateRequest request) {
        JourneyResponse response = journeyService.createPersonalJourney(memberId, request);
    ...
    public ApiResult<JourneyResponse> updatePersonalJourney(
            @MemberId Long memberId,
            @PathVariable Long journeyId,
            @Valid @RequestBody PersonalJourneyUpdateRequest request) {
        JourneyResponse response = journeyService.updatePersonalJourney(memberId, journeyId, request);
    ...
    // 귀가 여정 생성 (막차/데드라인 공통)
    @PostMapping("/home")
    public ApiResult<JourneyResponse> createHomeJourney(
            @MemberId Long memberId,
            @Valid @RequestBody HomeJourneyCreateRequest request) {
        JourneyResponse response = journeyService.createHomeJourney(memberId, request);
    ...
    public ApiResult<JourneyResponse> updateHomeJourney(
            @MemberId Long memberId,
            @PathVariable Long journeyId,
            @Valid @RequestBody HomeJourneyUpdateRequest request) {
        JourneyResponse response = journeyService.updateHomeJourney(memberId, journeyId, request);
