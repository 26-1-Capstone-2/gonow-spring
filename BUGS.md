...
### 🟠 버그50 — 프론트가 카카오 REST API 키로 카카오 로컬 API를 직접 호출(서버 프록시 없음) [영향 중간, 1차 조치 완료]
...
**1차 조치 완료(2026-08-21)**: 카카오 콘솔에서 키 재발급(복제 키 생성 후 기존 키 삭제) + 새 키를 `.env.local`(gitignore 대상)로 이전, `.env`에서는 제거. 깃허브 노출은 막았지만, **APK 디컴파일로 추출 가능한 근본 문제는 아직 남아있음**(어느 `.env*` 파일에서 왔든 `EXPO_PUBLIC_` 값은 빌드 시 앱 번들에 그대로 박히므로).
...
버그49 (플라스크 지각 패턴 학습 데이터가 로컬 JSON 파일 저장, 재배포 시 유실 위험) → 영향 낮음(우아한 폴백 있음), Redis 도입 검토 중 대조 사례로 발견
버그50 (프론트가 카카오 REST API 키로 카카오 로컬 API 직접 호출, 서버 프록시 없음) → 영향 중간, 깃허브 노출은 키 재발급+.env.local 이전으로 1차 조치 완료, APK 디컴파일 노출은 서버 프록시 신설 전까지 잔존
