...
### 🟡 버그49 — 플라스크 지각 패턴 학습 데이터가 로컬 JSON 파일에 저장됨(컨테이너 재배포 시 유실 위험) [영향 낮음, 우아한 폴백 있음]

→ 아래 버그49 상세 참고

---

### 🟠 버그50 — 프론트가 카카오 REST API 키로 카카오 로컬 API를 직접 호출(서버 프록시 없음) [영향 중간, 1차 조치 브랜치에만 반영·구 키 미폐기]
...
**파일**: `CounterClockEngine/gps_api/core/latency.py`(`load_records`/`save_record`)

**증상**: 사용자별 "지각 패턴" 학습 데이터(`recommended_buffer()`가 출발 여유시간 계산에 사용)가 DB가 아니라 플라스크 서버 로컬 디스크의 JSON 파일(`latency_{user_id}.json`)에 저장됨. 이 경로가 영구 볼륨에 마운트돼 있지 않다면, 컨테이너 재배포/재시작마다 전체 사용자의 학습 이력이 소실될 수 있음.

**영향이 낮은 이유**: `load_records`가 빈 결과를 반환해도 `recommended_buffer()`는 `DEFAULT_BUFFER_MIN`으로 조용히 폴백함(에러/크래시 없음) — 사용자 입장에선 "개인화된 여유시간"이 초기값으로 리셋되는 정도이지 알람 자체가 망가지지 않음.

**발견 경위**: 2026-08-21, Redis 도입 후보 조사 중 대조 사례(휘발성 데이터 vs durability 필요한 데이터 구분)로 확인.

**수정 방향**: MySQL에 `member_id` FK를 가진 지각 기록 테이블을 추가하고 스프링 경유로 저장/조회하거나, 최소한 EC2에서 이 디렉토리를 영구 볼륨으로 마운트해 컨테이너 재배포에도 살아남게 해야 함.
...
**파일**: `src/api/kakao.ts`(`searchAddress`, `searchPlace`), `.env`

**증상**: 프론트가 카카오 로컬 API(`dapi.kakao.com/v2/local/search/*`, 주소/장소 검색용)를 앱에서 **직접** 호출하고 있음. 이때 쓰는 `EXPO_PUBLIC_KAKAO_REST_API_KEY`는 REST API 키인데, 이 종류의 키는 네이티브 앱 키(패키지명+키해시로 보호)나 JS 키(도메인 화이트리스트로 보호)와 달리 **플랫폼 제한 보호가 없어서**, 앱(APK)에 실리는 순간 디컴파일로 누구나 추출해 자신의 프로젝트에 도용할 수 있음(카카오 API 일일 쿼터를 남이 소진시켜 실제 사용자 기능이 막힐 위험).

**추가로 발견된 부수 문제(1차 조치로 해결함)**: 이 키가 `.env`(gitignore 대상 아님)에 평문으로 커밋되어 있어서, Public 저장소인 `GoNow_Fronted`에 **깃허브를 통해서도 노출**되고 있었음. `.gitignore`에 `.env*.local`만 있고 `.env` 자체는 없었던 게 원인(2026-04-13 Expo 프로젝트 최초 생성 시 기본 템플릿 그대로, 2026-05-06 팀원이 이 키를 `.env`에 추가하면서 노출).

**1차 조치는 `feature/member-auth-system` 브랜치에만 반영, `dev`/`main`은 아직 미적용(2026-08-22 확인)**: 2026-08-21 커밋(`80f4b31`)에서 카카오 콘솔 새 키 발급 + `.env.local`(gitignore 대상)로 이전, 해당 브랜치의 `.env`에서는 제거했음. 하지만 이 브랜치가 아직 `dev`에 머지되지 않아서, 팀원이 실제로 pull하는 `dev`의 `.env`엔 **구 키가 여전히 평문으로 커밋된 채 남아있고 깃허브 노출도 그대로**임. 게다가 구 키는 카카오 콘솔에서도 아직 폐기 전(재발급만 하고 기존 키는 살려둔 상태)이라, 지금도 그 구 키로 카카오 API 호출이 가능함. 머지 시점엔 팀원에게 새 키를 별도 채널로 전달해야 함(안 그러면 `dev`를 pull한 팀원의 `searchAddress`/`searchPlace`가 401을 삼키고 빈 배열만 반환해 검색 기능이 조용히 깨짐) + 구 키 폐기도 별도로 챙겨야 함. 이 조치와 무관하게 **APK 디컴파일로 추출 가능한 근본 문제는 아직 남아있음**(어느 `.env*` 파일에서 왔든 `EXPO_PUBLIC_` 값은 빌드 시 앱 번들에 그대로 박히므로).

**수정 방향**: 스프링에 카카오 로컬 API 프록시 엔드포인트(예: `GET /api/places/search`) 신설 → 카카오 REST API 키를 스프링 서버 환경변수로만 보관(`flask.url`/SES 자격증명과 동일한 로컬/prod 분리 패턴) → 프론트 `kakao.ts`가 카카오를 직접 호출하는 대신 이 엔드포인트를 호출하도록 변경. (플라스크가 아니라 스프링에 추가해야 함 — 플라스크는 스프링만 바라보는 구조라 프론트 대상 엔드포인트는 스프링 담당.)
...
버그49 (플라스크 지각 패턴 학습 데이터가 로컬 JSON 파일 저장, 재배포 시 유실 위험) → 영향 낮음(우아한 폴백 있음)
버그50 (프론트가 카카오 REST API 키로 카카오 로컬 API 직접 호출, 서버 프록시 없음) → 영향 중간, 1차 조치(키 재발급+.env.local 이전)는 feature/member-auth-system 브랜치에만 반영되고 dev/main은 아직 미적용 + 구 키 폐기도 안 됨, APK 디컴파일 노출은 서버 프록시 신설 전까지 잔존
